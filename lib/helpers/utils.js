...
export function formatSocialLink(url){
  const urlSegment = url.split('/');
  const arrLength = urlSegment.length;
  const usernameIndex = urlSegment[arrLength-1] === "" ? arrLength - 2 : arrLength -1;
  return urlSegment[usernameIndex];
}