...
export default function Notes() {
  const { notes, selectedId, colors, seectedColor } = useSelector((state) => state.Notes);
        ...
        <Header className="isoHeader">
          {selectedId !== undefined ? (
            <div className="isoColorChooseWrapper">
              <ColorChoser colors={colors} changeColor={(v) => dispatch(changeColor(v))} seectedColor={seectedColor} />
              <span>
          ...
          <Button type="primary" className="isoAddNoteBtn" onClick={() => dispatch(addNote())}>
            <IntlMessages id="notes.addNote" />
          </Button>
        </Header>
        <Content className="isoNoteEditingArea">
          {selectedId !== undefined ? (
            <Textarea className="isoNoteTextbox" value={selectedNote.note} onChange={updateNote} autoFocus />
          ) : (
            ""
          )}
