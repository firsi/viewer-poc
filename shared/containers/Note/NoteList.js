...
export default function (props) {
  ...
  function singleNote(note) {
    ...
    return (
      <div className={`isoList ${activeClass}`} key={note.id}>
        <div className="isoNoteBGColor" style={{ width: "5px", background: colors[note.color] }} />
        <div className="isoNoteText" onClick={onChange}>
          <h3>{note.note}</h3>
          <span className="isoNoteCreatedDate">{timeDifference(note.createTime)}</span>
        </div>
        <Button className="isoDeleteBtn" icon={<CloseOutlined />} type="default" onClick={onDelete} />
      </div>
    );
  }
  ...
  return (
    <NoteListWrapper className="isoNoteListWrapper">
      <InputSearch placeholder="Search Notes" className="isoSearchNotes" value={search} onChange={onChange} />
      <div className="isoNoteList">
        {notes && notes.length > 0 ? (
          <Scrollbars style={{ height: "calc(100vh - 70px)" }}>{notes.map((note) => singleNote(note))}</Scrollbars>
        ) : (
