...
const { filterAction, selectMail, changeComposeMail, changeReplyMail, changeSearchString } = mailActions;
...
export default function MobileView(props) {
  ...
  const { allMails, selectedMail, filterAttr, composeMail, replyMail, searchString } = mail;
  ...
  return (
    <div style={{ height: "100%" }}>
          ...
          composeMail ? (
            <div className="isoSingleMailWrapper">
              <Scrollbars style={{ height: props.height - 70 }}>
                {composeMail ? (
                  <ComposeMail allMails={allMails} mobileView="true" changeComposeMail={changeComposeMail} />
                ) : (
                  singleMailComponent
                )}
              </Scrollbars>
      ...
      <MailDrawer>
        <ReactDrawer open={open} right={true} onChange={closeDrawer} style={{ position: "relative" }}>
          <span onClick={closeDrawer} style={{ position: "absolute", top: 4, right: 5 }}>
            <CloseIcon />
          </span>
          <div className="isoLeftWrapper">
            <ComposeBtn changeComposeMail={changeComposeMail} onDrawerClose={closeDrawer} />
