...
import { ChatWindow, ChatBox, ToggleViewProfile, MessageDialog } from "./Messages.styles";
      ...
      <ChatBox style={{ height: "100%" }}>
        <Modal visible={openCompose} onCancel={() => dispatch(toggleCompose())} title="Compose Message" footer={null}>
          <MessageDialog>
            ...
            <InputName users={users} setComposedId={(id) => dispatch(setComposedId(id))} className={className} />
            <ComposeMessage autosize={{ minRows: 5, maxRows: 9 }} showButton rows={8} />
          </MessageDialog>
        </Modal>
        {selectedChatRoom && (
          <ToggleViewProfile>
            <span onClick={() => dispatch(toggleViewProfile(selectedChatRoom.otherUserInfo))}>
