...
const ViewTable = ({ invoiceList }) => <Table columns={viewColumns} dataSource={invoiceList} pagination={false} />;
    ...
    },
    {
      ...
      render: (text, singleInvoice) => (
        <Input
          ...
          onChange={(event) => {
            editableInvoice.invoiceList[singleInvoice.key - 1].costs = stringToPosetiveInt(
              event.target.value,
              singleInvoice.costs
            );
    ...
    },
    {
      ...
      render: (text, singleInvoice) => (
        <Input
          ...
          onChange={(event) => {
            editableInvoice.invoiceList[singleInvoice.key - 1].qty = stringToPosetiveInt(
              event.target.value,
              singleInvoice.qty
            );
            editInvoice(updateValues(editableInvoice));
