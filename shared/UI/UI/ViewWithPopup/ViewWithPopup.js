...
export default function ViewWithPopup({ view, popup, noView = false, style, className }) {
  ...
  return (
    <Wrapper className={`${addAllClasses.join(" ")} ${showPopup ? "active" : ""}`} ref={ref}>
      ...
      <Container className="popup_container" showPopup={showPopup} onClick={() => setShowPopup(true)} style={style}>
        {view && !noView && view}
        {showPopup && (
          <div id="popup">
